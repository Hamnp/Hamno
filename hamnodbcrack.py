...
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : Hamno {x}{m}•{k}•{h}•{x}''')
#--------------------[ HAMNO ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© hamno : [green]Cookiedough[white] ©©©'))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{x}] M Cookies dane :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} cookie ba sarkawtwy kari krd ...{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s facebook swtawa!!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
